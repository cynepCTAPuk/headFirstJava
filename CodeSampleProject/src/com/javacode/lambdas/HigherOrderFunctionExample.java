...
public class HigherOrderFunctionExample {
    public static void main(String[] args) {
        ...
        persons.add(new RichPerson("Alex", "Black", 25, 50_000));
        persons.add(new RichPerson("John", "Green", 35, 75_000));
        persons.add(new RichPerson("Sam", "Brown", 40, 80_000));
        persons.add(new RichPerson("Tony", "Grey", 50, 90_000));
    ...
    private static void testPredicate(List<RichPerson> persons) {
        ...
        Predicate<RichPerson> isRich = x -> x.getSalary() >= 75_000;
    ...
    private static <T> List<T> findAll(List<T> elements, Predicate<T> predicate) {
        ...
        for (T element : elements) if (predicate.test(element)) filteredList.add(element);
        return filteredList;
    }
...
//        transform(persons, composeFunction).forEach(System.out::println);
    ...
    private static <T> Function<T, T> compose(Function<T, T>... functions) {
        ...
        for (Function<T, T> f : functions) result = result.andThen(f);
        return result;
    }

    private static <T, R> List<R> transform(List<T> elements, Function<T, R> function) {
        List<R> list = new ArrayList<>();
        for (T elem : elements) list.add(function.apply(elem));
        return list;
    }

    private static void testConsumer(List<RichPerson> persons) {
        System.out.println("...Testing consumer...");
//        f1.andThe(f2(x)) ==> f1(x) andThen f2(x);
        Consumer<RichPerson> rised = x -> x.setSalary(x.getSalary() * 1.1);
    ...
    private static <T> void processList(List<T> elements, Consumer<T> consumer) {
        for (T e : elements) consumer.accept(e);
    }
}
