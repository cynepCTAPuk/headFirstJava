package concurrency.javarush.task2708;
...
public class Solution {
    public static void main(String[] args) {
        ...
        for (int i = 0; i < 20; i++) {
            new Thread(() -> {
                for (int i1 = 0; i1 < 10; i1++) realEstate.revalidate();
            }, "RealEstateThread" + i).start();

            new Thread(() -> {
                for (int i12 = 0; i12 < apartments.length; i12++) apartments[i12].revalidate(true);
