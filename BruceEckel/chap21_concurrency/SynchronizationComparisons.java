//: chap21_concurrency/SynchronizationComparisons.java
...
abstract class Accumulator {
    ...
    report(Accumulator acc1, Accumulator acc2) {
        printf("%-23s: %5.2f\n", acc1.id + "/" +
                acc2.id, (double)acc1.duration/(double)acc2.duration);
    }
}
...
public class SynchronizationComparisons {
    ...
    static SynchronizedTest synchro = new SynchronizedTest();
    ...
    static void test() {
        ...
        lock.timedTest();
        atomic.timedTest();
        Accumulator.report(synchro, baseLine);
        Accumulator.report(lock, baseLine);
        Accumulator.report(atomic, baseLine);
        Accumulator.report(synchro, lock);
        Accumulator.report(synchro, atomic);
        Accumulator.report(lock, atomic);
    }
    public static void main(String[] args) {
        int iterations = 5; // Default
        if(args.length > 0) iterations = Integer.valueOf(args[0]); // Optionally change iterations
        // The first time fills the thread pool:
        print("Warmup");
        baseLine.timedTest();
        // Now the initial test doesn’t include the cost of starting the threads
        // for the first time. Produce multiple data points:
